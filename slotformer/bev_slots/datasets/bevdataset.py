...
class CarlaVoiceDataset(BaseIODataset):
    def __init__(
        ...
        **kwargs,
        ...
        print("Scenario nums: %d" % len(self.scenario_infos))
    ...
    def _extract_data_item(self, route_path, frame_id):
        ...
        # THESIS: load birdeye views in addition to other things
        context_len = 7 # context for stosavi. If we are at the beginning of the video, we take frame_id amount, further down the line we take the last two bevs
        ...
        # for fr in range(max(frame_id - context_len + 1, 0), max(frame_id + 1, 3)):
        #     bev_images.append(transform(
        #         self._load_image(
        #              os.path.join(route_path, "birdview", "%04d.jpg" % (fr))
        #             )
        #         )
        #     )
        ...
        # Add None padding
        # if frame_id < context_len:
        #     bev_images = (context_len - frame_id - 1) * [None]
        #     context_len = context_len - (context_len - frame_id - 1)
        #     logging.info(f"added {(context_len - frame_id - 1)} Nones now adding {context_len} real bevs to context for stosavi")

        # for frame in reversed(range(context_len)):
        #     img = self._load_image(
        #             os.path.join(route_path, "birdview", "%04d.jpg" % (frame_id - frame))
        #         )
        #     bev_images.append(transform(img))
            
        # birdeye_image = self._load_image(
        #     os.path.join(route_path, "birdview", "%04d.jpg" % frame_id)
        # )

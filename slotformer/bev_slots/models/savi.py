...
class StoSAVi(BaseModel):
        ...
        self.encoder = nn.Sequential(
            *[
                conv_norm_act(
                    ...
                    stride=(
                        2
                        if (i == 0 and self.resolution[0] == 64 and self.downsample)
                        else 1
                    ),  # 64) else 1,    # changed 128 -> 192
                    norm=self.enc_norm,
    ...
    # THESIS: This is the same as in slotformer. For some reason it was marked not implemented.
    ...
    def _get_encoder_out(self, img):
        ...
        img = img.float() # THESIS: converting to float (c10:half) to match bias data type
    ...
    def encode(self, img, prev_slots=None, return_attn=False):
        ...
        # N is context length for stosavi
        B, T, C, H, W = img.shape # Thesis: shape fix this once multiple images in img
        img = img.flatten(0, 1) # flattening batch and time dims
    ...
    # THESIS: this is from slotformer, added to the light savi due to it not being present

...
class SlotFormerParams(BaseParams):
    ...
    # data settings
    dataset = 'bev' # TEHSIS: change to our dataset
    data_root = '/scratch/project_2014099/data-lmdrive/data/test-dataset'
    ...
    # Slot Attention
    slot_dict = dict(
        num_slots=14,  # at most 5 objects per scene
        slot_size=256,
        slot_mlp_size=256,
        num_iterations=2,
    )
    ...
    loss_dict = dict(
        ...
        # THESIS: here it is None in our carformer config, however it seems none means that we use standard savi instead of stochastic, which we don't intend to do
    )
